...
import type { RankingBrandDTO } from '~/types/ranking-brand-dto'
...
export default function Brand() {
  ...
  const { data, isLoading, refetch } = useFetch<RankingBrandDTO>(
    ['get-brand-ranking-query', filter.SIZE, filter.VARIANT],
  ...
  const dataByPeriod: ByPeriod[] | false = useMemo(
    () =>
      data
        ? data[PERIOD.BRAND[filter.PERIOD!]].length === 0
          ? false
          : data[PERIOD.BRAND[filter.PERIOD!]].map((item, index) => ({
              ...item,
              id: item.marcaId,
    ...
    <Container>
      <ScrollView
        ...
        <Header.Root style={{ paddingHorizontal: 24 }}>
          <Header.Back>
            {selected
              ? `${selected.marcaNome.length > 20 ? `${selected.marcaNome.slice(0, 20)}...` : selected.marcaNome}`
              : 'MARCAS'}
          </Header.Back>
          <Header.Content />
        </Header.Root>
