...
import { RankingCategoryDTO } from '~/types/ranking-category-dto'
...
export default function Category() {
  ...
  const { data, isLoading, refetch } = useFetch<RankingCategoryDTO>(
    ['get-category-ranking-query', filter.SIZE, filter.VARIANT],
  ...
  const dataByPeriod: ByPeriod[] | false = useMemo(
    () =>
      data
        ? data[PERIOD.CATEGORY[filter.PERIOD!]].length === 0
          ? false
          : data[PERIOD.CATEGORY[filter.PERIOD!]].map((item, index) => ({
              ...item,
              id: item.categoriaId,
    ...
    <Container>
      <ScrollView
        ...
        <Header.Root style={{ paddingHorizontal: 24 }}>
          <Header.Back>
            {selected
              ? `${selected.categoriaNome.length > 20 ? `${selected.categoriaNome.slice(0, 20)}...` : selected.categoriaNome}`
              : 'CATEGORIAS'}
          </Header.Back>
          <Header.Content />
        </Header.Root>
            ...
            <Sheet.Root
              ...
              {selected ? (
                <View className="w-full p-10">
                  ...
                  <Link
                    asChild
                    href={`/details/vendacateegoria/codigoCategoria=${selected.id}/${selected.categoriaNome}`}>
                    <TouchableOpacity
                      activeOpacity={0.8}
