...
export default function Category() {
  ...
  const TOTAL = useMemo(
    () =>
      data
        ? data[PERIOD.CATEGORY[period!]].reduce(
  ...
  const DATA_BY_PERIOD = useMemo(() => {
    if (!(data && data[PERIOD.CATEGORY[period]].length !== 0)) return false

    return data[PERIOD.CATEGORY[period!]].map((item, index) => ({
      ...item,
      id: item.categoriaId,
  ...
  return (
    <Container>
      <Header.Root style={{ paddingHorizontal: 20 }}>
        <Header.Back>CATEGORIAS</Header.Back>
        <Header.Content />
      </Header.Root>
